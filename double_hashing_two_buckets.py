...
class DoubleHashingEmbedding(nn.Module):
    def __init__(self, num_buckets, embedding_dim, aggregation="sum"):
        ...
        # embedding layer
        self.embedding_b1 = nn.Embedding(num_buckets, embedding_dim)
        self.embedding_b2 = nn.Embedding(num_buckets, embedding_dim)
        nn.init.xavier_uniform_(self.embedding_b1.weight)
        nn.init.xavier_uniform_(self.embedding_b2.weight)
    ...
    def forward(self, features):
        ...
        h1_tensor = torch.tensor(h1, dtype=torch.long, device=self.embedding_b1.weight.device)
        h2_tensor = torch.tensor(h2, dtype=torch.long, device=self.embedding_b2.weight.device)

        # Get embeddings corresponding to both hashes
        E_h1 = self.embedding_b1(h1_tensor)
        E_h2 = self.embedding_b2(h2_tensor)

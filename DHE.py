...
def hash_function(value, seed):
    ...
    return int(hashlib.md5(f"{value}_{seed}".encode('utf8')).hexdigest(), 16) % 10000
...
# Simulating 1000 unique user IDs
user_ids = list(range(1, 1001))

# Random target values for training
target_values = [random.random() for _ in range(1000)]
...
for epoch in range(num_epochs):
 ...
 # Generating embedding for a new user ID
new_user_id = 54321
